...
namespace {
    void getAllAddressesTo(std::vector<IPvXAddress>& outvector)
    {
        outvector.clear();
            ...
            if (ift)
            {
                for (int j=0; j < ift->getNumInterfaces(); j++)
                {
                    ...
                    if (ie && !ie->isLoopback())
                    {
                        ...
                        if (ie->ipv4Data())
                        {
                            addr = IPvXAddress(ie->ipv4Data()->getIPAddress());
                            outvector.push_back(addr);
                        }
#endif
#ifdef WITH_IPv6
                        if (ie->ipv6Data())
                        {
                            ...
                            {
                                addr = IPvXAddress(ie->ipv6Data()->getAddress(k));
                                outvector.push_back(addr);
                            }
                        }
#endif
...
void PingTestApp::initialize(int stage)
{
    ...
    sleepDuration = par("sleepDuration").doubleValue();
