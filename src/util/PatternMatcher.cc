...
std::string PatternMatcher::debugStrFrom(int from)
{
    ...
    for (int k=from; k<(int)pattern.size(); k++)
    {
        ...
        switch (e.type)
        {
            case LITERALSTRING: result = result + "\"" + e.literalstring + "\""; break;
            case ANYCHAR: result += "?!"; break;
            case COMMONCHAR: result += "?"; break;
            case SET: result = result + "SET(" + e.setchars + ")"; break;
            case NEGSET: result = result + "NEGSET(" + e.setchars + ")"; break;
            case NUMRANGE: {char buf[100]; sprintf(buf, "%ld..%ld", e.fromnum, e.tonum); result += buf; } break;
